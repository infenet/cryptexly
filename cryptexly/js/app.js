...
app.controller('PMController', ['$scope', function (scope) {
    ...
    scope.getStore = function(success, force) {
        if( force == false && scope.cryptexly.HasStore() == false ) {
            alert("No store selected");
        }
        else {
            scope.setStatus("Loading passwords store ...");
            scope.cryptexly.SetStore( scope.store_id, function() {
                document.title = scope.cryptexly.store.title;
    ...
    scope.onDeleteStore = function() {
        if( scope.cryptexly.HasStore() == false ) {
            alert("No store selected");
        }
        else if( confirm( "Are you sure?" ) == true ) {
            scope.cryptexly.DeleteStore( scope.store_id, function() {
                scope.cryptexly.store = null;
                scope.store_id = null;
                scope.doSelectStore();
            },
            scope.errorHandler );
    ...
    scope.onNewStore = function() {
        ...
        if( store_title ) {
            scope.cryptexly.AddStore( store_title, function() {
                scope.setError(null);
                scope.doSelectStore();
            },
            scope.errorHandler );
    ...
    scope.setupTimeout = function() {
        for( var i = 0; i < this.cryptexly.records.length; i++ ) {
            var record = this.cryptexly.records[i];
    ...
    scope.doSelectStore = function() {
        scope.cryptexly.Stores(function(stores){
            document.title = "Cryptexly v" + VERSION;
            scope.setRoute(null);
            scope.delTimeout();
    ...
    scope.doLogin = function() {
        ...
        if( scope.setKey( $('#key').val(), persist ) == true ) { 
            scope.cryptexly.Login( username, password, persist, function(token) {
                scope.setError(null);
                scope.$apply();
                scope.route();
            },
    ...
    scope.onStoreTitleChanged = function(new_value) {
        ...
        scope.cryptexly.UpdateStore( scope.store_id, new_value, function(){
            scope.getStore(function(){}, true);
        },
        scope.errorHandler );
    };
            ...
            setTimeout( function() {
                ...
                record.Encrypt(scope.key).then(function(data){
                    ...
                    scope.showLoader("Adding record ...", function(){
                        ...
                        scope.cryptexly.AddRecord( r, data, function(record) {
    ...
    scope.getSecretById = function(id) {
        for( var i = 0; i < scope.cryptexly.records.length; i++ ) {
            var secret = scope.cryptexly.records[i];
            if( secret.id == id ){
                return secret;
            }
        }
    ...
    scope.onShowSecret = function(id) {
        ...
        scope.showLoader( "Buffering data ...", function() {
            ...
            scope.progressAt = new Date();
            scope.uploading = false;
            scope.cryptexly.GetRecordBuffer( secret.id, function(data){
                ...
                scope.showLoader( "Decrypting data ...", function() {
                    ...
                    record.Decrypt( secret.encryption, scope.key, data, 
                        function(){
                            ...
                            document.title = scope.cryptexly.store.title + " > " + secret.title;
        ...
        scope.showLoader("Encrypting record ...", function(){
            ...
            record.Encrypt(scope.key).then(function(data){
                ...
                scope.showLoader("Updating Record ...", function(){
                    ...
                    scope.cryptexly.UpdateRecord( r, data, function() {
    ...
    scope.onDelete = function() {
        ...
        if( confirm( "Delete this secret?" ) == true ) {
            scope.cryptexly.DeleteRecord(scope.secret, function(){ 
                scope.setSecret(null);
                scope.getStore( function() {
                    scope.$apply();
                });
        ...
        scope.showLoader("Encrypting record ...", function(){
            ...
            record.Encrypt(scope.key).then(function(data){
                ...
                scope.showLoader(action + " record ...", function(){
                    ...
                    scope.cryptexly.AddRecordTo( r, data, dest_store_id, function(record) {
                        if( move ) {
                            scope.cryptexly.DeleteRecord(scope.secret, function(){ 
                                scope.setSecret(null);
                                scope.getStore( function() {
    ...
    scope.updateServerStatus = function() {
        ...
        if( scope.cryptexly.config != null ) {
            var token_duration_minutes = scope.cryptexly.config.token_duration;
            var token_life = Date.now() - scope.cryptexly.token_time;
