...
var TPL_LOGIN_OK = T("login_ok",
	"On {{.When}} address {{.Address}} successfully logged into the Cryptexly server.")

var TPL_LOGIN_KO = T("login_ko",
	`On {{.When}} address <b>{{.Address}}</b> tried to log into the Cryptexly server with username 
...
var TPL_UPDATE = T("update",
	"You are running Cryptexly v{{.Version}} but v{{.NewVersion}} is available <a href=\"{{.Link}}\">for download</a>.")
